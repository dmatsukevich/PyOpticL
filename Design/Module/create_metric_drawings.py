...
inch = optomech.inch
bolts = [optomech.bolt_m2_5, optomech.bolt_m3, optomech.bolt_m4, optomech.bolt_m6]
            ...
            if isinstance(edge.Curve, Part.Circle):
                ...
                for bolt in bolts:
                    ...
                    if isclose(bolt["clear_dia"], 2 * radius):
                        ...
                        break
                    if isclose(optomech.bolt_m6["clear_dia"], 2 * radius): # these large bolts are only for bolts into optical table
                        label = "A"
                        break
                    if isclose(optomech.bolt_m4["clear_dia"], 2 * radius):
                        label = "B"
                        break
                    if isclose(optomech.bolt_m3["tap_dia"], 2 * radius):
                        label = "C"
                        break
                if label != None:
